...
void DeferredPipeline::destroyShadowFrameBuffers() {
...
void DeferredPipeline::updateCameraUBO(Camera *camera, bool hasOffScreenAttachments) {
...
void DeferredPipeline::updateShadowUBO(Camera *camera) {
    const auto scene = camera->getScene();
    const Light *mainLight = nullptr;
    if (scene->mainLightID) mainLight = scene->getMainLight();
    auto *device = gfx::Device::getInstance();
...
void DeferredPipeline::updateGlobalUBO() {
